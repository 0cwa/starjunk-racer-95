...
func _ready() -> void:
	...
	telemetry.sample(0.5, body, [
		{"grounded": true, "driven": true, "slip_angle_rad": deg_to_rad(5.0), "longitudinal_slip_ratio": 0.10, "wheel_angular_speed_rad_s": 20.0},
		{"grounded": true, "driven": true, "slip_angle_rad": deg_to_rad(-7.0), "longitudinal_slip_ratio": -0.20, "wheel_angular_speed_rad_s": -30.0},
		{"grounded": false},
		{"grounded": false},
	])
	...
	telemetry.sample(0.5, body, [
		{"grounded": true, "driven": true, "slip_angle_rad": deg_to_rad(9.0), "longitudinal_slip_ratio": 0.30, "wheel_angular_speed_rad_s": 40.0},
		{"grounded": true, "driven": true, "slip_angle_rad": deg_to_rad(3.0), "longitudinal_slip_ratio": -0.40, "wheel_angular_speed_rad_s": -50.0},
		{"grounded": true, "driven": true, "slip_angle_rad": deg_to_rad(-1.0), "longitudinal_slip_ratio": 0.05, "wheel_angular_speed_rad_s": 10.0},
		{"grounded": true, "driven": false, "slip_angle_rad": deg_to_rad(-2.0), "longitudinal_slip_ratio": 0.15, "wheel_angular_speed_rad_s": 25.0},
	])
	...
	_check(float(result.mean_driven_abs_longitudinal_slip_ratio) > 0.20, "driven slip should be tracked separately")
	_check(is_equal_approx(float(result.mean_free_abs_longitudinal_slip_ratio), 0.15), "free-wheel slip should be tracked separately")
