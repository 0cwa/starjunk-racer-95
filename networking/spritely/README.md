...
CI proves five things against the pinned Goblins 0.18 / Hoot 0.9 package families:
...
5. **Real-browser instantiation:** CI copies `reflect.js`, `reflect.wasm` and `wtf8.wasm` from the exact pinned Hoot package, serves them with the compiled Starjunk module over localhost, and requires headless Chromium to instantiate and run the module with the Starjunk browser host imports.
...
- `tests/browser-smoke.html` / `tests/browser-smoke.mjs` — minimal real-browser Hoot bootstrap.
...
The next slice is to turn the successfully instantiated browser module into a narrow race-domain bridge: create/join room, readiness/content binding, lifecycle state, and opaque capability references for Godot's `MultiplayerAdapter`, while keeping high-frequency snapshots on the separate realtime lane.
