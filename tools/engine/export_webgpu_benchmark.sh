...
GODOT_BIN="${1:?usage: export_webgpu_benchmark.sh GODOT_BIN TEMPLATE_ZIP OUTPUT_DIR}"
...
python3 - "$PROJECT_FILE" "$PRESET_FILE" "$TEMPLATE_ZIP" <<'PY'
import sys
...
new_main = 'run/main_scene="res://tests/perf/renderer_torture/renderer_torture.tscn"'
...
printf 'Exported WebGPU benchmark to %s\n' "$OUTPUT_DIR"
